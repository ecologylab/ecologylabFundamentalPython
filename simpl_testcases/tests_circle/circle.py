...
class Circle(unittest.TestCase):
    ...
    def setUp(self):
        self.scope = SimplTypesScope(Format.JSON, "circle_scope")
        fileReader = open("circle.xml", "r")
        self.pointXMLResult = fileReader.read()
        fileReader.close()
                
    def test_xml_run(self):
        simpl_object = self.scope.deserialize("circle.xml", Format.XML)
    ...
    def test_json_run(self):
        simpl_object = self.scope.deserialize("circle.json", Format.JSON)
