...
class Circle(unittest.TestCase):
    ...
    def setUp(self):
        self.scope = SimplTypesScope(Format.JSON, "diamond_scope")
        fileReader = open("diamond.xml", "r")
        self.pointXMLResult = fileReader.read()
        fileReader.close()
                
    def test_xml_run(self):
        simpl_object = self.scope.deserialize("diamond.xml", Format.XML)
        if False:
    ...
    def test_json_run(self):
        simpl_object = self.scope.deserialize("diamond.json", Format.JSON)
        if False:
            json_text = deserialize_from_file("diamond.json")
            print(json_text)
