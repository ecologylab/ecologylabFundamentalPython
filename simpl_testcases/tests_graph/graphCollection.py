...
class Circle(unittest.TestCase):
    ...
    def setUp(self):
        self.scope = SimplTypesScope("JSON", "graphCollection_scope")
        fileReader = open("graphCollection.xml", "r")
        self.pointXMLResult = fileReader.read()
        fileReader.close()
                
    def test_xml_run(self):
        simpl_object = self.scope.deserialize("graphCollection.xml", "XML")
        if False:
    ...
    def test_json_run(self):
        simpl_object = self.scope.deserialize("graphCollection.json", "JSON")
        if False:
