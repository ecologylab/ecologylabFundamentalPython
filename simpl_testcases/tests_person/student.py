...
class Point(unittest.TestCase):
    ...
    def setUp(self):
        self.scope = SimplTypesScope(Format.JSON, "student_scope")
        self.pointXMLResult = open("student.xml", "r").read()
        
    def test_xml_run(self):
        simpl_object = self.scope.deserialize("student.xml", Format.XML)
    ...
    def test_json_run(self):
        simpl_object = self.scope.deserialize("student.json", Format.JSON)
        json_text = deserialize_from_file("student.json")
        print(json_text)
